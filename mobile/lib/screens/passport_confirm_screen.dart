  ...
  Future<void> _confirmAndProceed() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (mounted) {
          final formRaw = GoRouterState.of(context).uri.queryParameters['form'] ?? 'ds160';
          final formNormalized = formRaw.toLowerCase().replaceAll('-', ''); // ds160 / ds260
          final targetPath = formNormalized == 'ds260' ? '/kyc/ds260' : '/kyc/chat';
