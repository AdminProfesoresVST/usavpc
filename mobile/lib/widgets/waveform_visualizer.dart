...
class _WaveformVisualizerState extends State<WaveformVisualizer> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    // IDLE STATE: Gentle animation (User requested "line with moving colors")
    if (!widget.isActive) {
        ...
        builder: (context, _) {
          return Row(
            ...
            children: List.generate(widget.barCount, (index) {
              // Gentle height variation for idle state
              final idleHeight = 8.0 + sin((_colorAnimation.value * 2 * pi) + index) * 3.0;
