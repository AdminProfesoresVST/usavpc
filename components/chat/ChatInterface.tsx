        ...
        if (!isTyping && currentQuestion?.type !== 'select') {
            ...
            const { object, submit, isLoading: isStreaming, error: streamError } = experimental_useObject({
                ...
                onFinish: ({ object: finalObj }) => {
                    ...
                    // Replace the "Streaming" placeholder with Final
                    // Actually, while streaming, we should update the message list?
                    // Or just use `object` to render the last message?
                    // For simplicity: Add to messages on finish.
