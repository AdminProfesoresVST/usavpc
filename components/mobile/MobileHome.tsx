    ...
    const handlePurposeSelect = async (purpose: string) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Fallback: Just go to scan (assuming user might be logged in or handling it there)
                // Actually, if failed, we shouldn't proceed. But for UI demo:
                router.push(`/${locale}/scan`);
            }
        } catch (e) {
            console.error(e);
